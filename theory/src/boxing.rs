...
use crate::gen::{TheoryError,TheoremGen,peel_foralls};
...
pub trait TheoremBox : Sized {
    ...
    fn box_a2(a: FormulaVars, b: FormulaVars, c: FormulaVars, boxes: &[Boxing]) -> Result<Self, TheoryError>;
    fn box_a4(a: FormulaVars, b: FormulaVars, x: &str, boxes: &[Boxing]) -> Result<Self, TheoryError>;
...
fn install_hyps(mut t: Theorem, boxes: &[Boxing], mut xs: &[String]) -> Result<Theorem, TheoryError> {
...
mod test {
    ...
    fn x()->ExprVars { ExprVars::var("x") }
    fn y()->ExprVars { ExprVars::var("y") }

    #[test]
    fn x_imp_y_imp_a2() {
        Theorem::box_a2(x().eq(x()), y().eq(y()), x().eq(y()), &[
                       Boxing::Var("x".to_owned()),
                       Boxing::Hyp(x().eq(num(0))),
                       Boxing::Var("y".to_owned()),
                       Boxing::Hyp(y().eq(num(0)))
        ]).unwrap()
            .check("@x(x=0 -> @y(y=0 -> ((x=x -> y=y -> x=y) -> (x=x -> y=y) -> (x=x -> x=y))))").unwrap();
    }
    ...
    fn x_y_a2() {
        Theorem::box_a2(x().eq(x()), y().eq(y()), x().eq(y()), &[
            ...
            .check("@x(@y(((x=x -> y=y -> x=y) -> (x=x -> y=y) -> (x=x -> x=y))))").unwrap();  
    }
    #[test]
    fn x_imp_y_imp_mp() {
        let bs = &[
        ...
        let t1 = Theorem::box_a2(x().eq(x()), y().eq(y()), x().eq(x()), bs).unwrap()
            .check("@x(x=0 -> @y(y=0 -> ((x=x -> y=y -> x=x) -> (x=x -> y=y) -> (x=x -> x=x))))").unwrap();

        let t2 = Theorem::box_a1(x().eq(x()), y().eq(y()), bs).unwrap()
            .check("@x(x=0 -> @y(y=0 -> (x=x -> y=y -> x=x)))").unwrap();

        t1.box_mp(t2, bs).unwrap().check("@x(x=0 -> @y(y=0 -> (x=x -> y=y) -> (x=x -> x=x)))").unwrap();
    }
